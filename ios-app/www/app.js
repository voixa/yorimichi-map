/* ============================================================
 * Yorimichi Map — iOS App (Capacitor)
 * ------------------------------------------------------------
 * このファイルは ios-app/www/app.js (Web版とは別管理)
 * 初回コピー: 2026-06-07 ios-app/scripts/init-www.mjs 経由
 *
 * Web版との差分:
 *   - Service Worker 登録を削除 (バンドル内アセット・更新は App Store経由)
 *   - Stripe Checkout (startStripeCheckout / startSubscriptionCheckout) を no-op化
 *     → 規約 4.5.4 / 3.1.1 対応。StoreKit 2 IAP 実装で差し替え予定
 *   - UI 非表示は style.css の html[data-platform="ios"] ルールで担当
 * ------------------------------------------------------------
...
(function () {
  ...
  // ---------- iOS Platform marker ----------
  // ios-app/www/ 配下のコードは iOS版固定。CSS と JS のフックを起動時に立てる。
  if (typeof document !== 'undefined') {
    try { document.documentElement.setAttribute('data-platform', 'ios'); } catch (e) {}
    document.addEventListener('DOMContentLoaded', () => {
      try {
        document.documentElement.setAttribute('data-platform', 'ios');
        document.body && document.body.classList.add('is-ios-app');
      } catch (e) {}
    });
  }
  ...
  const APP_NAME = 'YorimichiMap-iOS/0.1';
...
${trkPts}
  ...
  // 🍎 iOS版: Apple 規約により外部決済禁止。StoreKit 2 IAP 実装まで no-op
  async function startSubscriptionCheckout() {
    showToast('💎 アプリ版のプレミアム加入は近日対応予定です', 'info', 3000);
  }
  ...
  // 島パス生成と同一のcos補正equirectangular投影（定数はスクリプト出力を焼き込み＝ピンと海岸線が一致）
  function latLngToPct(lat, lng) {
    const k = 0.788011, minPx = 101.9753, minPy = -45.5515, scale = 21.07863, offX = 16, offY = 46.949;
    const X = (lng * k - minPx) * scale + offX;
    const Y = (-lat - minPy) * scale + offY;
    return { x: Math.max(1, Math.min(99, X / 300 * 100)), y: Math.max(1, Math.min(99, Y / 400 * 100)) };
  }
  ...
  // 実海岸線ベースの日本列島パス（viewBox 300x400・ピンと同一投影で生成）— 地図描画とシェア画像で共用
  // 出典: johan/world.geo.json (Natural Earth派生・パブリックドメイン) を cos補正equirectangularで投影
  const CJM_ISLAND_PATHS = [
    'M102.9,287.3 L105.0,294.5 L95.6,307.3 L88.8,300.5 L80.3,305.4 L75.9,317.7 L65.1,311.7 L65.2,301.7 L74.4,289.2 L83.8,291.6 L90.7,282.7 L102.9,287.3 Z',
    'M208.1,224.2 L201.9,241.0 L204.8,251.6 L196.1,266.4 L174.9,276.4 L145.7,277.7 L122.0,301.7 L110.9,293.6 L110.2,277.9 L81.3,282.5 L61.6,292.4 L42.2,292.8 L59.0,308.4 L48.0,344.2 L37.2,353.1 L29.2,344.9 L33.3,325.9 L22.7,319.7 L16.0,305.3 L31.7,298.8 L40.4,285.5 L57.1,274.6 L69.3,260.2 L102.4,253.9 L120.1,258.2 L137.5,220.8 L148.6,230.8 L173.0,209.8 L182.4,201.6 L192.8,175.8 L190.0,152.1 L197.0,138.8 L214.7,134.9 L223.7,164.1 L223.2,181.2 L207.9,202.5 L208.1,224.2 Z',
    'M256.9,76.0 L268.6,80.5 L280.3,71.5 L284.0,95.2 L259.4,101.0 L244.8,121.9 L218.7,107.5 L209.7,130.6 L191.2,130.9 L188.9,109.9 L197.1,93.7 L214.9,92.5 L219.7,63.4 L224.6,46.9 L244.1,68.9 L256.9,76.0 Z',
  ];
...
${hashtag}`;
  ...
  async function init() {
    ...
    // 🍎 iOS版: Service Worker は使わない (アセットはバンドル内・更新は App Store 経由)

    // 🍎 iOS版: Stripe Checkout 戻り処理は無効 (IAP に移行予定)
